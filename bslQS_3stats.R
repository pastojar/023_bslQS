...
# variability among the runoff from the local RGs
locRG_stats <- list()
for ( i_locRG in 1:3 ) {
  ...
  hlp <- locRG_stats[[i_locRG]][["overview_ev"]][["table_NSE"]]
  colnames(hlp) <- paste0(i_locRG_name, "__", locRG_names[ !locRG_names %in%  i_locRG_name ]  )
  if ( i_locRG == 1 ) {
    locRG_NSE_tab <- hlp
  } else {
    locRG_NSE_tab <- cbind( locRG_NSE_tab, hlp )  
  }
}
locRG_NSE_tab$var <- apply( locRG_NSE_tab, 1, mean )

events.subsets [[ paste0( "locRG_Qvar_", i_agg, "_1" ) ]] <- periods$st[ as.character(periods$st) %in% rownames( locRG_NSE_tab[ order(locRG_NSE_tab$var, decreasing = T), ] ) [ 1:8 ] ]
events.subsets [[ paste0( "locRG_Qvar_", i_agg, "_2" ) ]] <- periods$st[ as.character(periods$st) %in% rownames( locRG_NSE_tab[ order(locRG_NSE_tab$var, decreasing = T), ] ) [ 9:16 ] ]
events.subsets [[ paste0( "locRG_Qvar_", i_agg, "_3" ) ]] <- periods$st[ as.character(periods$st) %in% rownames( locRG_NSE_tab[ order(locRG_NSE_tab$var, decreasing = T), ] ) [ 17:24 ] ]
...
save( merged_stats, locRG_NSE_tab, file = paste0(getwd(), "/outputs/", package, "_3stats.Rdata") )
